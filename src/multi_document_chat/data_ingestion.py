...
class DocumentIngestor:
    ...
    def __init__(self,data_dir: str = "/Users/MadhavGunampalli/Documents/llm-ops/document-portal/data/multi_doc_chat", faiss_dir: str = "faiss_index", session_id: str | None = None):
        try:
            ...
            self.log.info("MultiDocIngestor initialized", temp_path=str(self.data_dir), faiss_path=str(self.faiss_dir))
        except Exception as e:
            self.log.error("Failed to initialize MultiDocIngestor", error=str(e))
            raise DocumentPortalException("Initialization error in MultiDocIngestor", sys)
    
    def ingestFiles(self, uploaded_files):
        try:
            documents=[]
            for uploaded_file in uploaded_files:
                ext = Path(uploaded_file.name).suffix.lower()
                if ext not in self.SUPPORTED_EXTENSIONS:
                    self.log.warning("Unsupported file ", filename=uploaded_file.name)
                    continue
